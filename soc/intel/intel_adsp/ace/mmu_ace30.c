...
#define MEM_MAP_CONST_C(val) \
	((uint32_t)RPO_SET((uintptr_t)(val), CONFIG_XTENSA_CACHED_REGION))

#define MEM_MAP_CONST_UC(val) \
	((uint32_t)RPO_SET((uintptr_t)(val), CONFIG_XTENSA_UNCACHED_REGION))
...
const struct xtensa_mmu_range xtensa_soc_mmu_ranges[] = {
...

	{
		...
		.attrs = XTENSA_MMU_PERM_W | XTENSA_MMU_MAP_SHARED,
		.name = "shared heap",
	},
