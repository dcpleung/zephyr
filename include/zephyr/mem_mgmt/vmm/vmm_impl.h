 ...
 *
 * APIs for the virtual memory manager implementation which handles
 * virtual memory.
 */
...
#include <zephyr/mem_mgmt/vmm/vmm_impl_arch.h>

#elif defined(CONFIG_MEM_MGMT_VMM_IMPL_CUSTOM)

/* Custom implementation utilizing the functions below. */
...
/**
 * Initialize the virtual memory manager implementation.
 *
 * Perform necessary steps to initialize the virtual memory manager.
 */
 ...
 * are assumed to be aligned to page aligned (e.g. CONFIG_MMU_PAGE_SIZE
 * if MMU is used).
 *
 ...
 *
 * @retval 0 On success
 * @retval -EINVAL Invalid arguments are given (for example, zero size)
 * @retval -ENOTSUP The requested cache/attribute combination in @a flags
 *                  is not supported by the implementation
 * @retval -ENOMEM The memory mapping fails
 * @retval -errno Other negative error codes may be returned by
 *                the implementation for other failure modes
 */
 ...
 *
 * Behavior when providing unaligned addresses/sizes is undefined, these
 * are assumed to page aligned (e.g. CONFIG_MMU_PAGE_SIZE if MMU is used).
 *
 ...
 *
 * Some page frames within system RAM are reserved and are not available
 * for use.
 *
 * This function should mark all relevant entries in k_mem_page_frames
 * with K_PAGE_FRAME_RESERVED. This function is called at early system
 * initialization.
 */
