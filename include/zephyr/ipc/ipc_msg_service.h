...
struct ipc_msg_service_cb {
	 ...
	 * @param[in] priv Private data.
	 */
	void (*received)(uint8_t msg_type, const void *data, void *priv);
	 ...
	 * @param[in] data Pointer to event related data.
	 * @param[in] priv Private data.
	 */
	void (*event)(uint8_t evt_type, const void *data, void *priv);
 ...
 * @param[in] data Pointer to the message to be sent.
 *
 ...
 *		     invalid length, unsupported message type, ...)
 * @retval -EBUSY when the instance is busy.
 ...
 * @retval other errno codes depending on the implementation of the backend.
 */
int ipc_msg_service_send(struct ipc_msg_ept *ept, uint8_t msg_type, const void *data);
