...
static int send(const struct device *instance, void *token, uint8_t msg_type, const void *data)
{
	...
	msg = (struct ipc_msg_type_cmd *)data;

	msg->cmd += dev_config->offset;

	dev_data->cfg->cb.event(IPC_MSG_EVT_REMOTE_DONE, NULL, dev_data->cfg->priv);

	dev_data->cfg->cb.received(msg_type, msg, dev_data->cfg->priv);
