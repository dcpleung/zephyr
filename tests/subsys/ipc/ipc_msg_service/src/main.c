...
static void received_cb(uint8_t msg_type, const void *data, void *priv)
{
	uintptr_t expected = (uintptr_t)priv;
	struct ipc_msg_type_cmd *msg = (struct ipc_msg_type_cmd *)data;
...
static void event_cb(uint8_t evt_type, const void *data, void *priv)
{
...
ZTEST(ipc_msg_service, test_ipc_msg_service)
{
...
ZTEST(ipc_msg_service, test_ipc_msg_wrong_message_type)
{
	const struct device *dev_10;
	struct ipc_msg_ept ept_10;
	struct ipc_msg_type_cmd_arg msg = {.cmd = 10, .arg = 20};
	int ret;
	...
	ret = ipc_msg_service_send(&ept_10, IPC_MSG_TYPE_CMD_ARG, (const void *)&msg);
	zassert_equal(ret, -EBADMSG, "ipc_msg_service_send() should fail but not");
