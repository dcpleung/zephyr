...
extern struct ipc_ept ipc_ept;

struct test_priv_data {
	struct intel_adsp_ipc_ept_priv_data priv_data;
	volatile uint32_t host_dt;
};

static struct test_priv_data priv_data;
...
void clock_ipc_receive_cb(const void *data, size_t len, void *priv)
{
	if (len == INTEL_ADSP_IPC_CB_MSG) {
		...
		struct test_priv_data *tpd = (struct test_priv_data *)priv;

		tpd->host_dt = msg->data;

		tpd->priv_data.cb_ret = INTEL_ADSP_IPC_CB_RET_OKAY;
	}
}
...
ZTEST(intel_adsp, test_clock_calibrate)
{
	...
	host_dt = &priv_data.host_dt;

	ret = ipc_service_register_endpoint(INTEL_ADSP_IPC_HOST_DEV, &ipc_ept, &clock_ipc_ept_cfg);
...
#else
	ret = ipc_service_deregister_endpoint(&ipc_ept);
