 ...
 * where the low level mapping fails but unmapping works fine. This allows
 * the recovery to reclaim all memory.
 */
ZTEST(mem_map_api, test_anon_mem_map_anon_good_recovery)
{
...
/**
 * Test the failure recovery path for anonymous memory in k_mem_map()
 * when both the low level mapping and unmapping routines fail.
 * This tests the path where mapping fails, and the path reclaiming
 * memory also fails.
 */
ZTEST(mem_map_api, test_anon_mem_map_anon_bad_recovery)
{
	size_t free_mem_before, free_mem_after_map;
	char *mapped, *last_successful_mapped, *mapped_adjusted;
	int op_num_succeed;
	...
	/* In this test, when a mapping fails, th recovery path will retry to
	 * reclaim memory. However, the unmapping routinue also fails which
	 * prevents memory from being reclaimed.
	 */

	/* Try to fake a failure immediately without mapping any pages.
	 *
	 * Note that unmapping must not fail for two times as
	 * the low level unmapping is called twice for guard pages.
	 */
	printk("Try immediately failed mapping...\n");
	vmm_fake_failure_setup(true, 0, true, 2, -ENOMEM);
	...
	zassert_equal(op_num_succeed, 2, "should not have succeed in any unmappings (%d)",
		      op_num_succeed);
	...
	/* Try to fake a failure when allocating the 3rd page, out of NUM_PAGES_TO_MAP.
	 * And fail to reclaim the 2nd page. Remember that low level unmapping is also
	 * called to unmap the guard pages. So we need to add 2 to number of unmapping
	 * tries.
	 */
	printk("Try halfway failed mapping...\n");
	vmm_fake_failure_setup(true, 2, true, 3, -ENOMEM);
	...
	op_num_succeed = vmm_unmap_num_succeed_get();
	zassert_equal(op_num_succeed, 3, "should have succeed in some unmappings (%d)",
		      op_num_succeed);

	/* Since we have mapped 2 pages and unmapped 1, the number of page mapped should be 1. */
	match_num_mapped(last_successful_mapped, NUM_PAGES_TO_MAP, 1);

	free_mem_after_map = k_mem_free_get();
	printk("- Free memory after failed mapping: %zu\n", free_mem_after_map);
	zassert_not_equal(free_mem_before, free_mem_after_map,
			  "failed mapping and recovery should take up some memory");

	/* Return to normal operation */
	vmm_fake_failure_reset();

	/* Map again to see if this mapping returns different address as
	 * the first mapping done above. Since the recovery path fails,
	 * the allocated virtual memory region should not have been released.
	 */
	mapped = k_mem_map(MAPPING_SZ, K_MEM_PERM_RW);
	zassert_not_null(mapped, "memory mapping should not fail");

	mapped_adjusted = last_successful_mapped;
	mapped_adjusted -= MAPPING_SZ;
	mapped_adjusted -= CONFIG_MMU_PAGE_SIZE * 2; /* 2 guard pages */

	zassert_equal(mapped, mapped_adjusted,
		      "virtual memory region should not have been released (%p != %p)", mapped,
		      last_successful_mapped);

	k_mem_unmap(mapped, MAPPING_SZ);

	/* Since exactly 1 page is mapped and not reclaimed, we need to adjust
	 * the equality for that.
	 */
	free_mem_after_map = k_mem_free_get();
	printk("- Free memory after cleanup: %zu\n", free_mem_after_map);
	zassert_equal(free_mem_before - CONFIG_MMU_PAGE_SIZE, free_mem_after_map,
		      "cleanup should free up all used memory");
}
...
/* For CPUs with incoherent cache under SMP, tests which read/write
 * memory may not work correctly if the test thread jumps between CPUs.
 * So use the test infrastructure to limit the test to 1 CPU.
 */
