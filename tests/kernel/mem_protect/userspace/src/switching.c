...
static void run_switching(int num_kernel_threads)
{
	...
	for (i = 0; i < NUM_THREADS; i++) {
		...
		k_thread_create(&threads[i], threads_stacks[i], STACKSIZE, thread_fn,
	...
	for (i = 0; i < NUM_THREADS; i++) {
		...
		if ((i % 2) == 0) {
			...
			zassert_equal(part_b_loops[i], 0,
				       "part_b_loops[%i] should be zero but not", i);
		} else {
			loops = part_b_loops[i];

			zassert_equal(part_a_loops[i], 0,
				       "part_a_loops[%i] should be zero but not", i);
		}

		zassert_equal(loops, TIMES_SWITCHING,
			      "thread %u has not done enough loops (%u != %u)",
			      i, loops, TIMES_SWITCHING);
	}
...
void *switching_setup(void)
{
	...
	zassert_equal(
		...
		0, "failed to initialize memory domain A");

	zassert_equal(
		k_mem_domain_init(&domain_b, ARRAY_SIZE(parts_b), parts_b),
		0, "failed to initialize memory domain B");
...
ZTEST_SUITE(userspace_domain_switching, NULL, switching_setup,
	    switching_before, NULL, NULL);
