...
/**
 * @brief Register I3C device with controller.
 *
				 ...
				 struct i3c_device_desc *desc)
{
	...
	ret = i3c_dev_list_target_add(dev_list, desc);
	if (ret == 0) {
		LOG_DBG("I3C device registered: %p", desc);
	} else {
		LOG_DBG("I3C device registration failed: %p", desc);
	}
...
/**
 * @breif Register I2C device with controller.
 *
				     ...
				     struct i3c_i2c_device_desc *desc)
{
	...
	ret = i3c_dev_list_i2c_devce_add(dev_list, desc);
	if (ret == 0) {
		LOG_DBG("I2C device registered: %p", desc);
	} else {
		LOG_DBG("I2C device registration failed: %p", desc);
	}
...
static int mcux_i3c_do_daa(const struct device *dev)
{
	...
	do {
		...
		if ((mcux_i3c_state_get(base) == I3C_MSTATUS_STATE_DAA) &&
			...
			ret = i3c_dev_list_daa_addr_helper(&data->device_list, pid,
							   false, false,
							   &target, &dyn_addr);
			if (ret != 0) {
				goto out_daa;
			...
			i3c_dev_list_addr_mark_i3c(&data->device_list, dyn_addr);
			...
			if ((target->static_addr != 0U) && (dyn_addr != target->static_addr)) {
				i3c_dev_list_addr_mark_free(&data->device_list, dyn_addr);
			}
...
/**
 * @brief Initialize I3C bus.
 *
 * @param dev Pointer to controller device driver instance.
 *
 * @return 0 if successful, negative if errors.
 */
...
//	while (tmp == 1);
	...
	return i3c_bus_init(dev, &data->device_list);
}
...
static const struct i3c_driver_api mcux_i3c_driver_api = {
	...
	.i2c_device_register = mcux_i3c_i2c_dev_register,

	.i3c_xfers = mcux_i3c_transfer,
	.i2c_xfers = mcux_i3c_i2c_transfer,
