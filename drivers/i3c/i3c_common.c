			    ...
			    struct i3c_device_desc *target)
{
	...
	/* Add target descriptor to I3C device list */
			       ...
			       struct i3c_i2c_device_desc *i2c_dev)
{
	...
	if (!i3c_dev_list_addr_is_free(dev_list, i2c_dev->addr)) {
	...
	i3c_dev_list_addr_mark_i2c(dev_list, i2c_dev->addr);
				 ...
				 uint8_t *addr)
{
	...
	desc = i3c_dev_list_pid_find(dev_list, pid);
	...
	if (desc->init_dynamic_addr != 0U) {
		if (i3c_dev_list_addr_is_free(dev_list,
	 ...
	 */
	dyn_addr = i3c_addr_slots_status_next_free_find(&dev_list->addr_slots);
...
int i3c_bus_init(const struct device *dev, struct i3c_dev_list *dev_list)
{
	int ret = 0;
	bool need_daa = true;
