...
int stmemsc_i3c_read(const struct i3c_device_desc **stmemsc,
		     uint8_t reg_addr, uint8_t *value, uint8_t len)
{
	struct i3c_device_desc *target = (void *)*stmemsc;

	return i3c_burst_read(target, reg_addr, value, len);
}

int stmemsc_i3c_write(const struct i3c_device_desc **stmemsc,
		      uint8_t reg_addr, uint8_t *value, uint8_t len)
{
	struct i3c_device_desc *target = (void *)*stmemsc;
